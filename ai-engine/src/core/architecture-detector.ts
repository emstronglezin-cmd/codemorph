...
export class ArchitectureDetector {
  ...
  private async aiDetect(ctx: ConversionContext, ast: ASTResult): Promise<{ pattern: ArchResult['pattern']; tokensUsed: number }> {
    const prompt = `Analyze this ${ctx.sourceFramework} project structure and return JSON:
{"pattern": "mvc|clean|feature-sliced|bloc|provider|monolith|layered|unknown"}
...
Return ONLY valid JSON.`;
    ...
    try {
      const res = await this.openai.chat.completions.create({
        model: appConfig.defaultModel, temperature: 0.1, max_tokens: 100,
        messages: [{ role: 'user', content: prompt }],
      });
      const data = JSON.parse(res.choices[0]?.message?.content ?? '{}') as { pattern?: ArchResult['pattern'] };
      return { pattern: data.pattern ?? 'unknown', tokensUsed: res.usage?.total_tokens ?? 0 };
    } catch {
